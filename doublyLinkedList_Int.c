...
void DLList_Int_MoveBefore(node_t* knot) {
...
void DLList_Int_MoveAfter(knot_t* knot) {
...
void DLList_Int_MoveToIndex(knot* knot, int index) {
...
void DLList_Intern_InsertBefore(node_t* node1, node_t* node2) {
    ...
    // Insert node1 in front of node2
    node1->prev = node2->prev;
    node1->next = node2;
    node2->prev->next = node1;
    node2->prev = node1
